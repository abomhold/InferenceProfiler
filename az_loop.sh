...
for l in {1..100}; do
  ...
  for az in 0 1 2; do
    ...
    for IP in $ALL_IPS; do
      ...
      until timeout 1 bash -c "</dev/tcp/${IP}/22"; do
        sleep 2
      done
    done
    ...
    if ! curl -s -f --retry-connrefused --retry-all-errors --retry 300 --retry-delay 2 --retry-max-time 600 \
