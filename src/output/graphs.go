...
func GenerateGraphsFromOutputFile(outputPath string) error {
	...
	if err != nil {
		return fmt.Errorf("failed to read %s rows: %w", filepath.Ext(outputPath), err)
	}
	if len(records) < 2 {
		return fmt.Errorf("need at least 2 records to generate graphs")
	}
...
func buildSeries(records []map[string]interface{}) []*Series {
	// Find all numeric columns (excluding timestamps and JSON fields)
	...
	for _, r := range records {
		...
		for col := range cols {
			...
			if v, ok := toFloatOk(r[col]); ok {
				// Try metric-specific timestamp first (e.g., "vCpuTime" -> "vCpuTimeT")
				ts := fallbackTs
...
func buildHistograms(records []map[string]interface{}) []*Histogram {
	...
	for _, r := range records {
		...
		// For each histogram, add a data point
...
// getHistogramJSON extracts the histogram JSON string from a record
func getHistogramJSON(r map[string]interface{}) string {
	// Try multiple possible field names (case variations)
	for _, key := range []string{"vllmHistogramsJson", "VLLMHistogramsJSON", "vllmHistogramsJSON"} {
...
func renderStaticInfo(sessionUUID string, info map[string]interface{}) string {
	...
	b.WriteString(`<div class="static-info-header"><h1>Profiler Report</h1>`)
