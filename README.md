...
The profiler aggregates metrics from multiple collectors. Below is the complete list of all metrics organized by device
type, with their JSON keys, units, sources, and descriptions.

All dynamic metrics include a timestamp field prefixed with `t` (e.g., `tvCpuTimeUserMode` is the timestamp for
`vCpuTimeUserMode`).
...
|--------------|---------------|---------|-----------------------------------------------------------------------|---------------------------------------------------------------|
| VM ID        | `vId`         | string  | 0. `/sys/class/dmi/id/product_uuid`<br>2. Fallback: `/etc/machine-id` | VM/instance identifier.                                       |
| Current Time | `currentTime` | seconds | Python: `int(time.time())`                                            | Unix epoch timestamp (seconds since 1969-01-01 00:00:00 UTC). |
...
|--------------------------|-----------------------|-------|---------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------|
| **CPU Time (Aggregate)** |                       |       |                                                                                                                                             |                                                                      |
| Total CPU Time           | `vCpuTime`            | cs    | File: `/proc/stat`<br>Logic: Sum of `user` + `system` fields on `cpu` line.                                                                 | Total CPU time (user + kernel mode). 1 cs = 10ms.                    |
| User Mode Time           | `vCpuTimeUserMode`    | cs    | File: `/proc/stat`<br>Field: `cpu` line, col 1 (`user`).                                                                                    | Time spent executing user-space processes.                           |
| Kernel Mode Time         | `vCpuTimeKernelMode`  | cs    | File: `/proc/stat`<br>Field: `cpu` line, col 3 (`system`).                                                                                  | Time spent executing kernel code on behalf of processes.             |
| Idle Time                | `vCpuIdleTime`        | cs    | File: `/proc/stat`<br>Field: `cpu` line, col 4 (`idle`).                                                                                    | Time CPU spent in idle state (no work).                              |
| I/O Wait Time            | `vCpuTimeIOWait`      | cs    | File: `/proc/stat`<br>Field: `cpu` line, col 5 (`iowait`).                                                                                  | Time CPU spent waiting for I/O operations to complete.               |
| IRQ Time                 | `vCpuTimeIntSrvc`     | cs    | File: `/proc/stat`<br>Field: `cpu` line, col 6 (`irq`).                                                                                     | Time spent servicing hardware interrupts.                            |
| Soft IRQ Time            | `vCpuTimeSoftIntSrvc` | cs    | File: `/proc/stat`<br>Field: `cpu` line, col 7 (`softirq`).                                                                                 | Time spent servicing software interrupts (deferred work).            |
| Nice Time                | `vCpuNice`            | cs    | File: `/proc/stat`<br>Field: `cpu` line, col 2 (`nice`).                                                                                    | Time spent running niced (low priority) user processes.              |
| Steal Time               | `vCpuSteal`           | cs    | File: `/proc/stat`<br>Field: `cpu` line, col 8 (`steal`).                                                                                   | Time stolen by hypervisor for other VMs (virtualized environments).  |
| **Performance Counters** |                       |       |                                                                                                                                             |                                                                      |
| Context Switches         | `vCpuContextSwitches` | count | File: `/proc/stat`<br>Field: `ctxt` line.                                                                                                   | Total context switches across all CPUs since boot.                   |
| Load Average             | `vLoadAvg`            | ratio | File: `/proc/loadavg`<br>Field: 1st column (1-min avg).                                                                                     | 1-minute system load average (runnable + uninterruptible processes). |
| CPU Frequency            | `vCpuMhz`             | MHz   | File: `/sys/devices/system/cpu/*/cpufreq/scaling_cur_freq`<br>Logic: Average of all cores / 1000.<br>Fallback: `/proc/cpuinfo` (`cpu MHz`). | Current average CPU frequency across all cores.                      |
...
| CPU Model       | `vCpuType`       | File: `/proc/cpuinfo`<br>Field: `model name`.                                                               | Processor model name string.                       |
| Cache Sizes     | `vCpuCache`      | File: `/sys/devices/system/cpu/cpu*/cache/index*`<br>Files used: `level`, `type`, `size`, `shared_cpu_map`. | Object with L1d, L1i, L2, L3 cache sizes in bytes. |
...
|---------------------|-------------------|-------|----------------------------------------------------------------------------------------------|----------------------------------------------------------|
| **Physical Memory** |                   |       |                                                                                              |                                                          |
| Total Memory        | `vMemoryTotal`    | bytes | File: `/proc/meminfo`<br>Field: `MemTotal` (converted kB to B).                              | Total physical RAM installed.                            |
| Free Memory         | `vMemoryFree`     | bytes | File: `/proc/meminfo`<br>Field: `MemAvailable` (fallback: `MemFree` + `Buffers` + `Cached`). | Available memory (includes reclaimable buffers/cache).   |
...
| Buffer Memory       | `vMemoryBuffers`  | bytes | File: `/proc/meminfo`<br>Field: `Buffers`.                                                   | Memory used by kernel buffers for block device I/O.      |
| Cached Memory       | `vMemoryCached`   | bytes | File: `/proc/meminfo`<br>Logic: `Cached` + `SReclaimable`.                                   | Memory used by page cache (file data cache).             |
| Memory Percent      | `vMemoryPercent`  | %     | Derived: `(Total - Available) / Total * 100`.                                                | Percentage of total RAM currently in use.                |
| **Swap Space**      |                   |       |                                                                                              |                                                          |
| Swap Total          | `vSwapTotal`      | bytes | File: `/proc/meminfo`<br>Field: `SwapTotal`.                                                 | Total swap space available.                              |
| Swap Free           | `vSwapFree`       | bytes | File: `/proc/meminfo`<br>Field: `SwapFree`.                                                  | Unused swap space.                                       |
| Swap Used           | `vSwapUsed`       | bytes | Derived: `SwapTotal` - `SwapFree`.                                                           | Currently used swap space.                               |
| **Page Faults**     |                   |       |                                                                                              |                                                          |
| Page Faults         | `vPgFault`        | count | File: `/proc/vmstat`<br>Field: `pgfault`.                                                    | Total minor page faults (page in memory but not mapped). |
| Major Page Faults   | `vMajorPageFault` | count | File: `/proc/vmstat`<br>Field: `pgmajfault`.                                                 | Major page faults requiring disk I/O to resolve.         |

**Static Info:**

| Field        | JSON Key            | Detailed Source                              | Description                  |
|--------------|---------------------|----------------------------------------------|------------------------------|
| Memory Total | `vMemoryTotalBytes` | File: `/proc/meminfo`<br>Field: `MemTotal`.  | Total physical RAM in bytes. |
| Swap Total   | `vSwapTotalBytes`   | File: `/proc/meminfo`<br>Field: `SwapTotal`. | Total swap space in bytes.   |
...
**Aggregation Logic:** Iterates over `/proc/diskstats`. Includes lines matching regex
`^(sd[a-z]+|hd[a-z]+|vd[a-z]+|xvd[a-z]+|nvme\d+n\d+|mmcblk\d+)$`. Sums values across all matched devices.

| Metric Name          | JSON Key                | Units   | Detailed Source                        | Description                                          |
|----------------------|-------------------------|---------|----------------------------------------|------------------------------------------------------|
| **Sector Counts**    |                         |         |                                        |                                                      |
| Sectors Read         | `vDiskSectorReads`      | sectors | File: `/proc/diskstats`<br>Column: 5.  | Total sectors read (1 sector = 512 bytes typically). |
| Sectors Written      | `vDiskSectorWrites`     | sectors | File: `/proc/diskstats`<br>Column: 9.  | Total sectors written.                               |
| **Byte Counts**      |                         |         |                                        |                                                      |
...
| **Operation Counts** |                         |         |                                        |                                                      |
| Successful Reads     | `vDiskSuccessfulReads`  | count   | File: `/proc/diskstats`<br>Column: 3.  | Number of read operations completed successfully.    |
| Successful Writes    | `vDiskSuccessfulWrites` | count   | File: `/proc/diskstats`<br>Column: 7.  | Number of write operations completed successfully.   |
| Merged Reads         | `vDiskMergedReads`      | count   | File: `/proc/diskstats`<br>Column: 4.  | Adjacent read requests merged for efficiency.        |
| Merged Writes        | `vDiskMergedWrites`     | count   | File: `/proc/diskstats`<br>Column: 8.  | Adjacent write requests merged for efficiency.       |
| **Timing**           |                         |         |                                        |                                                      |
| Read Time            | `vDiskReadTime`         | ms      | File: `/proc/diskstats`<br>Column: 6.  | Total time spent on read operations.                 |
| Write Time           | `vDiskWriteTime`        | ms      | File: `/proc/diskstats`<br>Column: 10. | Total time spent on write operations.                |
| I/O Time             | `vDiskIOTime`           | ms      | File: `/proc/diskstats`<br>Column: 12. | Total time spent doing I/O (wall clock).             |
| Weighted I/O Time    | `vDiskWeightedIOTime`   | ms      | File: `/proc/diskstats`<br>Column: 13. | I/O time weighted by number of pending operations.   |
| **Queue**            |                         |         |                                        |                                                      |
| I/Os in Progress     | `vDiskIOInProgress`     | count   | File: `/proc/diskstats`<br>Column: 11. | Number of I/O operations currently in flight.        |
...
**Aggregation Logic:** Iterates over `/proc/net/dev`. Skips first 2 header lines. Skips interface `lo`. Sums columns for
all other interfaces.

| Metric Name         | JSON Key               | Units | Detailed Source                                      | Description                                     |
|---------------------|------------------------|-------|------------------------------------------------------|-------------------------------------------------|
| **Byte Counters**   |                        |       |                                                      |                                                 |
| Bytes Received      | `vNetworkBytesRecvd`   | bytes | File: `/proc/net/dev`<br>Column: 0 (Receive bytes).  | Cumulative bytes received on all interfaces.    |
| Bytes Sent          | `vNetworkBytesSent`    | bytes | File: `/proc/net/dev`<br>Column: 8 (Transmit bytes). | Cumulative bytes transmitted on all interfaces. |
| **Packet Counters** |                        |       |                                                      |                                                 |
| Packets Received    | `vNetworkPacketsRecvd` | count | File: `/proc/net/dev`<br>Column: 1.                  | Total packets received.                         |
| Packets Sent        | `vNetworkPacketsSent`  | count | File: `/proc/net/dev`<br>Column: 9.                  | Total packets transmitted.                      |
| **Error Counters**  |                        |       |                                                      |                                                 |
| Receive Errors      | `vNetworkErrorsRecvd`  | count | File: `/proc/net/dev`<br>Column: 2.                  | Receive errors (CRC, framing, etc.).            |
| Send Errors         | `vNetworkErrorsSent`   | count | File: `/proc/net/dev`<br>Column: 10.                 | Transmit errors.                                |
| **Drop Counters**   |                        |       |                                                      |                                                 |
| Receive Drops       | `vNetworkDropsRecvd`   | count | File: `/proc/net/dev`<br>Column: 3.                  | Packets dropped on receive (buffer overflow).   |
| Send Drops          | `vNetworkDropsSent`    | count | File: `/proc/net/dev`<br>Column: 11.                 | Packets dropped on transmit.                    |
...
|------------------------|----------------------|--------|------------------------------------------------------------------------------------------------------|-------------------------------------------------------------|
| **Identification**     |                      |        |                                                                                                      |                                                             |
| Container ID           | `cId`                | string | File: `/proc/self/cgroup` (parses for `/docker/` or `/kubepods/`)<br>Fallback: `os.uname().nodename` | Container ID (Docker/Kubernetes short ID or "unavailable"). |
...
| **CPU Metrics**        |                      |        |                                                                                                      |                                                             |
| Total CPU Time         | `cCpuTime`           | ns     | v1: `cpuacct/cpuacct.usage`<br>v2: `cpu.stat` (key `usage_usec` * 1000)                              | Total CPU time consumed by all tasks in cgroup.             |
| User Mode Time         | `cCpuTimeUserMode`   | cs     | v1: `cpuacct/cpuacct.stat` (key `user`)<br>v2: `cpu.stat` (key `user_usec` / 10000)                  | CPU time in user mode.                                      |
| Kernel Mode Time       | `cCpuTimeKernelMode` | cs     | v1: `cpuacct/cpuacct.stat` (key `system`)<br>v2: `cpu.stat` (key `system_usec` / 10000)              | CPU time in kernel mode.                                    |
| Processor Count        | `cNumProcessors`     | count  | Python: `os.cpu_count()`                                                                             | Number of CPU processors available to container.            |
| Per-CPU Time           | `cCpu{i}Time`        | ns     | v1: `cpuacct/cpuacct.usage_percpu`<br>v2: Not supported                                              | CPU time on processor i (cgroup v1 only).                   |
| **Memory Metrics**     |                      |        |                                                                                                      |                                                             |
| Memory Used            | `cMemoryUsed`        | bytes  | v1: `memory/memory.usage_in_bytes`<br>v2: `memory.current`                                           | Current memory usage by processes in cgroup.                |
| Memory Max Used        | `cMemoryMaxUsed`     | bytes  | v1: `memory/memory.max_usage_in_bytes`<br>v2: `memory.peak`                                          | Peak memory usage (high watermark).                         |
| **Disk I/O**           |                      |        |                                                                                                      |                                                             |
| Disk Read Bytes        | `cDiskReadBytes`     | bytes  | v1: `blkio/blkio.throttle.io_service_bytes` (sum 'Read')<br>v2: `io.stat` (sum `rbytes`)             | Bytes read by cgroup from block devices.                    |
| Disk Write Bytes       | `cDiskWriteBytes`    | bytes  | v1: `blkio/blkio.throttle.io_service_bytes` (sum 'Write')<br>v2: `io.stat` (sum `wbytes`)            | Bytes written by cgroup to block devices.                   |
| **Network**            |                      |        |                                                                                                      |                                                             |
| Network Bytes Received | `cNetworkBytesRecvd` | bytes  | File: `/proc/net/dev`<br>(Read from inside container namespace)                                      | Bytes received (from container's network namespace).        |
| Network Bytes Sent     | `cNetworkBytesSent`  | bytes  | File: `/proc/net/dev`<br>(Read from inside container namespace)                                      | Bytes transmitted (from container's network namespace).     |
...
| **Threading**        |                                |        |                                                                        |                                                |
| Thread Count         | `pNumThreads`                  | count  | File: `/proc/[pid]/stat`<br>Field index: 19 (0-indexed base: 18)       | Number of threads in this process.             |
| **CPU Time**         |                                |        |                                                                        |                                                |
| User Mode Time       | `pCpuTimeUserMode`             | cs     | File: `/proc/[pid]/stat`<br>Field index: 13                            | CPU time scheduled in user mode.               |
| Kernel Mode Time     | `pCpuTimeKernelMode`           | cs     | File: `/proc/[pid]/stat`<br>Field index: 14                            | CPU time scheduled in kernel mode.             |
| Children User Time   | `pChildrenUserMode`            | cs     | File: `/proc/[pid]/stat`<br>Field index: 15                            | Cumulative user time of waited-for children.   |
| Children Kernel Time | `pChildrenKernelMode`          | cs     | File: `/proc/[pid]/stat`<br>Field index: 16                            | Cumulative kernel time of waited-for children. |
| **Context Switches** |                                |        |                                                                        |                                                |
| Voluntary Switches   | `pVoluntaryContextSwitches`    | count  | File: `/proc/[pid]/status`<br>Key: `voluntary_ctxt_switches`           | Context switches due to blocking (I/O, sleep). |
| Involuntary Switches | `pNonvoluntaryContextSwitches` | count  | File: `/proc/[pid]/status`<br>Key: `nonvoluntary_ctxt_switches`        | Context switches due to preemption.            |
| **I/O**              |                                |        |                                                                        |                                                |
| Block I/O Delays     | `pBlockIODelays`               | cs     | File: `/proc/[pid]/stat`<br>Field index: 41                            | Aggregated time waiting for block I/O.         |
| **Memory**           |                                |        |                                                                        |                                                |
| Virtual Memory       | `pVirtualMemoryBytes`          | bytes  | File: `/proc/[pid]/stat`<br>Field index: 22                            | Total virtual address space size.              |
| Resident Set Size    | `pResidentSetSize`             | bytes  | File: `/proc/[pid]/statm`<br>Field: 1 (pages) * Page Size (4096)       | Physical memory currently mapped (RSS).        |
...
The collector parses the Prometheus text response. It uses a regex
`^([a-zA-Z0-9_:]+)(?:\{(.+)\})?\s+([0-9\.eE\+\-]+|nan|inf|NaN|Inf)$` to extract values.
...
**Histogram Format:** Histograms are exported as JSON objects with bucket boundaries as keys and cumulative counts as
values:
